...
export const RepnotesFeatureTable = (props: RepnotesTableInterface) => {
                                    ...
                                    <TableRow hover key={row.name}>
                                        {props.columns.map((column) => {
                                            ...
                                            <TableCell key={column.field} component="th" scope="row" style={{ ...column.cellStyle, padding:'4px 16px'}} >
                                                ...
                                                {column.field === 'subFeature' && value}
                                                {column.field === 'price' && value}
                                                {column.field === 'action' 
                                                ?   <Box style={{ display: 'flex', flexDirection: 'row' }}>
                                                        <RepnotesActionButton onClick={() => props.onClickEdit((row.id ? row.id : row.name) as string, props.type)} type='modify'/>
                                                        {
                                                            !props.disabled &&
                                                            <RepnotesActionButton onClick={() => props.onClickDelete((row.id ? row.id : row.name) as string, props.type)} type='delete'/>
                                                        }
                                                    </Box>
                                                : null
                                                }
