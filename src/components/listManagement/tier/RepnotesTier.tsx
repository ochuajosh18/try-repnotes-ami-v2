...
class RepnotesTier extends Component<TierProps> {

    componentDidMount = () => {
        if (this.props.system.redirectPage.redirectTo !== '/tier') {
    ...
    _validateRole = async() => {
        if(this.props.system.session.userDetails.role !== 'SUPER ADMIN') await this.props.setTierCompanyFilter({selectedCompanyId: this.props.system.session.userDetails.companyId as string});
        this._loadTable();
    } 


    _loadTable = () => {
        if(this.props.tierState.selectedCompanyId !== '') this.props.getTierList(this.props.system, this.props.tierState.selectedCompanyId);
    }
    ...
    _deleteTier = () => {
        if(this.props.tierState.selectedCompanyId !== ''){
            this.props.deleteTier(this.props.dialog.docId, this.props.system.session.token, this.props.tierState.selectedCompanyId);
    ...
    _companyFilter = async(value: string ) => {
        await this.props.setTierCompanyFilter({selectedCompanyId: value as string});
        this._loadTable(); 
    }
    ...
    render = () => {
        const { tierList, loading, selectedCompanyId } = this.props.tierState;
        ...
        return (
            <Box className="repnotes-content">
                ...
                <RepnotesDialog 
                    ...
                    onClick={this._deleteTier.bind(this)}
