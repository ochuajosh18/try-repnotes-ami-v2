...
interface UnmetNeedsDateRangeProps {
    startDate: string;
    endDate: string;
    onDateChange: (start?: Date, end?: Date) => void;
}

const UnmetNeedsDateRange = (props: UnmetNeedsDateRangeProps) => {
    const [pickerOpen, setPickerOpen] = useState(false)
    return (
        <Grid item xs={3}>
            <Box width="100%" display="flex" position="relative" justifyContent="center" flexDirection="column">
                <RepnotesInput
                    id="repnotes-unmetneeds-report-date-ranges"
                    type="daterange"
                    label="Date Range"
                    labelPosition="top"
                    disabled={false}
...
export default UnmetNeedsDateRange